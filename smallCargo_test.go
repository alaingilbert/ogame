...
func TestSmallCargoSpeed(t *testing.T) {
	sc := newSmallCargo()
	assert.Equal(t, 6000, sc.GetSpeed(Researches{CombustionDrive: 2}))
	assert.Equal(t, 8000, sc.GetSpeed(Researches{CombustionDrive: 6}))
	assert.Equal(t, 8000, sc.GetSpeed(Researches{CombustionDrive: 6, ImpulseDrive: 4}))
