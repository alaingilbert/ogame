...
func TestExtractResourcesMobile(t *testing.T) {
	...
	res := NewExtractorV6().ExtractResources(pageHTMLBytes)
	assert.Equal(t, 7325851, res.Metal)
	assert.Equal(t, 1695823, res.Crystal)
	assert.Equal(t, 1835627, res.Deuterium)
	assert.Equal(t, -2827, res.Energy)
	assert.Equal(t, 19500, res.Darkmatter)
}
...
func TestExtractResearchV7(t *testing.T) {
	...
	assert.Equal(t, 2, res.EnergyTechnology)
	assert.Equal(t, 4, res.LaserTechnology)
	assert.Equal(t, 0, res.IonTechnology)
	assert.Equal(t, 0, res.HyperspaceTechnology)
	assert.Equal(t, 0, res.PlasmaTechnology)
	assert.Equal(t, 5, res.CombustionDrive)
	assert.Equal(t, 4, res.ImpulseDrive)
	assert.Equal(t, 0, res.HyperspaceDrive)
	assert.Equal(t, 4, res.EspionageTechnology)
	assert.Equal(t, 1, res.ComputerTechnology)
	assert.Equal(t, 3, res.Astrophysics)
	assert.Equal(t, 0, res.IntergalacticResearchNetwork)
	assert.Equal(t, 0, res.GravitonTechnology)
...
func TestExtractResourcesBuildingsV7(t *testing.T) {
	...
	assert.Equal(t, 2, res.MetalMine)
	assert.Equal(t, 1, res.CrystalMine)
	assert.Equal(t, 2, res.DeuteriumSynthesizer)
	assert.Equal(t, 3, res.SolarPlant)
	assert.Equal(t, 0, res.FusionReactor)
	assert.Equal(t, 0, res.SolarSatellite)
	assert.Equal(t, 2, res.MetalStorage)
	assert.Equal(t, 3, res.CrystalStorage)
	assert.Equal(t, 1, res.DeuteriumTank)
}
...
func TestExtractPlanet_no(t *testing.T) {
	...
	assert.Equal(t, 12800, planet.Diameter)
	assert.Equal(t, -24, planet.Temperature.Min)
	assert.Equal(t, 16, planet.Temperature.Max)
	assert.Equal(t, 0, planet.Fields.Built)
	assert.Equal(t, 188, planet.Fields.Total)
...
func TestExtractPlanet_sk(t *testing.T) {
	...
	assert.Equal(t, 12800, planet.Diameter)
	assert.Equal(t, -12, planet.Temperature.Min)
	assert.Equal(t, 28, planet.Temperature.Max)
	assert.Equal(t, 0, planet.Fields.Built)
...
func TestExtractAttacksACS2(t *testing.T) {
	...
	assert.Equal(t, 3, attacks[0].Ships.Battlecruiser)
	assert.Equal(t, GroupedAttack, attacks[1].MissionType)
	assert.Equal(t, 106734, attacks[1].AttackerID)
...
func TestExtractUserInfos_no(t *testing.T) {
	...
	assert.Equal(t, 0, infos.Points)
	assert.Equal(t, 49, infos.Rank)
	assert.Equal(t, 50, infos.Total)
}
...
func TestExtractUserInfos_sk(t *testing.T) {
	...
	assert.Equal(t, 0, infos.Points)
	assert.Equal(t, 89, infos.Rank)
	assert.Equal(t, 90, infos.Total)
}
...
func TestExtractUserInfos_gr(t *testing.T) {
	...
	assert.Equal(t, 0, infos.Points)
	assert.Equal(t, 162, infos.Rank)
	assert.Equal(t, 163, infos.Total)
}
...
func TestSystemDistance(t *testing.T) {
	assert.Equal(t, 3175, flightSystemDistance(499, 35, 30, false))

	assert.Equal(t, 2795, flightSystemDistance(499, 1, 2, true))
	assert.Equal(t, 2795, flightSystemDistance(499, 1, 499, true))
	assert.Equal(t, 2890, flightSystemDistance(499, 1, 3, true))
	assert.Equal(t, 2890, flightSystemDistance(499, 1, 498, true))
	assert.Equal(t, 2890, flightSystemDistance(499, 498, 1, true))
}
...
func TestDistance(t *testing.T) {
	assert.Equal(t, 1015, Distance(Coordinate{1, 1, 3, PlanetType}, Coordinate{1, 1, 6, PlanetType}, 6, 499, true, true))
	assert.Equal(t, 2890, Distance(Coordinate{1, 1, 3, PlanetType}, Coordinate{1, 498, 6, PlanetType}, 6, 499, true, true))
	assert.Equal(t, 20000, Distance(Coordinate{6, 1, 3, PlanetType}, Coordinate{1, 498, 6, PlanetType}, 6, 499, true, true))
	assert.Equal(t, 5, Distance(Coordinate{6, 1, 3, PlanetType}, Coordinate{6, 1, 3, MoonType}, 6, 499, true, true))
}
