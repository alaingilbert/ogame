...
func TestExtractPreferences(t *testing.T) {
	...
	prefs = ExtractPreferences(pageHTMLBytes)
	assert.Equal(t, 2, prefs.NbProbes)
	assert.True(t, prefs.DeactivateChatBar)
	assert.True(t, prefs.DeactivateOutlawWarning)
	assert.False(t, prefs.ShowMobileVersion)
	assert.True(t, prefs.ShowAlternativeDropdowns)
	assert.True(t, prefs.ActivateAutofocusHighscores)
	assert.Equal(t, 3, prefs.AlwaysShowEvents)
	assert.Equal(t, 3, prefs.SortPlanetsBy)
	assert.Equal(t, 1, prefs.SortingSequence)
	assert.False(t, prefs.HighlightPlanetInformation)
	assert.False(t, prefs.AnimatedDetailDisplay)
	assert.False(t, prefs.AnimatedOverview)
	assert.True(t, prefs.NotesInExtraWindow)
	assert.True(t, prefs.CombatReportsInExtraWindow)
	assert.True(t, prefs.HidePicturesInReports)
	assert.Equal(t, 50, prefs.DisplayedMessagePerPage)
	assert.False(t, prefs.AuctioneerNotification)
	assert.True(t, prefs.CreateEconomyMessages)
	assert.False(t, prefs.DetailedActivityDisplay)
	assert.True(t, prefs.PreserveGalaxySystemWithPlanetChange)

	pageHTMLBytes, _ = ioutil.ReadFile("samples/preferences_reverse_mobile.html")
	prefs = ExtractPreferences(pageHTMLBytes)
	assert.Equal(t, 2, prefs.NbProbes)
	assert.False(t, prefs.DeactivateChatBar) // no mobile
	assert.True(t, prefs.DeactivateOutlawWarning)
	assert.True(t, prefs.ShowMobileVersion)
	assert.True(t, prefs.ShowAlternativeDropdowns)
	assert.True(t, prefs.ActivateAutofocusHighscores)
	assert.Equal(t, 3, prefs.AlwaysShowEvents)
	assert.Equal(t, 3, prefs.SortPlanetsBy)
	...
	assert.False(t, prefs.AnimatedDetailDisplay)
	assert.False(t, prefs.AnimatedOverview)
	assert.False(t, prefs.NotesInExtraWindow)         // no mobile
	assert.False(t, prefs.CombatReportsInExtraWindow) // no mobile
	assert.True(t, prefs.HidePicturesInReports)
	assert.Equal(t, 50, prefs.DisplayedMessagePerPage)
	assert.False(t, prefs.AuctioneerNotification)
	assert.True(t, prefs.CreateEconomyMessages)
